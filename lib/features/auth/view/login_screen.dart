...
import '../viewmodel/auth_session_viewmodel.dart';

// View: 이제 API를 직접 호출하지 않는다. 로그인 성공 후 필요한 후속 조회
// (내 정보, 공휴일)를 어떤 순서로 호출할지만 조율(orchestrate)하고,
// 각 상태의 판단/저장은 해당 ViewModel에 맡긴다.
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _handleLogin() async {
    ...
    try {
      await context.read<AuthSessionViewModel>().login(
        _employeeNumberController.text.trim(),
        _passwordController.text,
      );

      if (!mounted) return;
      await context.read<ProfileViewModel>().fetchMyInfo();
