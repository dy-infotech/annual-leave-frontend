...
| ADM004_M01 | 사원 사번 조회 | `SearchEmployeeNumberScreen` | search_employee_number_screen.dart | `/search_employee_number_screen` |
| ADM004_D01 | 사원 상세 | `EmployeeDetailScreen` | employee_detail_screen.dart | - |

## 그룹 구성 기준

- **LVE002 (휴가 목록 조회)**: 내 휴가(M01)·전직원(M02)·관리자 검색(M03) 목록 화면이 공용 상세 화면(D01)을 공유하므로 하나의 그룹으로 묶었다. 결재 대기(LVE003_M01)도 LVE002_D01로 진입한다.
- **ADM004 (사번 조회)**: 사번 조회(M01)에서 사원 상세(D01)로 진입한다. 두 화면 모두 드로어의 관리자 전용 메뉴에서만 접근하는 사원 정보 관리 기능이므로 EMP가 아닌 ADM으로 분류했다. EMP에는 직원 본인용 화면(내 정보)만 둔다.
